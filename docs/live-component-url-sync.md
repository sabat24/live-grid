...
## Suggested discussion points for symfony/ux

1. **Is omitting default/initial prop values from the URL a desired feature?** I have tests and a working implementation; Livewire’s `keep` option was cited positively in #2141.
2. **Should `popstate` be part of the core URL binding feature?** `pushHistoryState` without hydration on back seems incomplete ([#3149](https://github.com/symfony/ux/issues/3149)).
3. **Multi-instance ergonomics:** Is auto-namespacing via live-id preferable to manual `alias` per instance?
4. **Form filters:** Should this stay application-level, or is there interest in a companion package?

### Suggested PR title

> Add `omitDefault` UrlMapping and `popstate` hydration for url-bound LiveProps
